...
if build_histogram: # se a caixa de seleção for selecionada
    ...
    fig2 = px.histogram(car_data, x="odometer")  # criar histograma
    st.plotly_chart(fig2, use_container_width=True)  # exibir um gráfico Plotly interativo

if build_dis: # se a caixa de seleção for selecionada
    st.write('Criando um histograma para a coluna odometer')

    fig3 = px.scatter(car_data, x="odometer", y="price")  # criar histograma
    st.plotly_chart(fig3, use_container_width=True)  # exibir um gráfico Plotly interativo
